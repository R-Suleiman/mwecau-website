...
@section('content')
    ...
    <div class="text-gray-900 uppercase rounded-md py-4 px-7 mt-7">
        ...
        Health Center News/Events</h1>
    </div>
    <div class="text-end">
        <div class="">

        </div>
        <a href="{{ route('health-center.create-news-event') }}"
                        ...
                        @foreach ($newsEvents as $newsEvent)
                            <tr class="hover:bg-gray-100 border border-gray-400">
                                ...
                                <td
                                    ...
                                    <a
                                        href="{{ route('health-center.edit-news-event', $newsEvent->title) }}">{!! Str::limit($newsEvent->title, '40') !!}</a>
                                </td>

                                <td class="px-4 py-2 flex items-center justify-center space-x-2">
                                    <a href="{{ route('health-center.edit-news-event', $newsEvent->title) }}"
                                    ...
                                    <form action="{{ route('health-center.destroy-news-event', $newsEvent->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
