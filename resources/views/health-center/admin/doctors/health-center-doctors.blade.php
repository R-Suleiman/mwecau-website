...
@section('content')
    ...
    <div class="text-gray-900 uppercase rounded-md py-4 px-7 mt-7">
        <h1
            class="headerFavFont text-2xl font-semibold bg-clip-text text-transparent bg-gradient-to-r from-purple-700 via-purple-500 to-purple-400">
            Health Center Doctors</h1>
    </div>
    <div class="text-end">
        <div class="">

        </div>
        <a href="{{ route('health-center.create-doctor') }}"
                ...
                <tbody class="bg-white">
                    ...
                    @if ($doctors->isNotEmpty())
                        @foreach ($doctors as $doctor)
                            <tr class="hover:bg-gray-100 border border-gray-400">
                                ...
                                <td
                                    ...
                                    <a
                                        href="{{ route('health-center.edit-doctor', $doctor->name) }}">{!! Str::limit($doctor->name, '40') !!}</a>
                                </td>
                        ...
                        <td class="px-4 py-2 flex items-center justify-center space-x-2">
                            <a href="{{ route('health-center.edit-doctor', $doctor->name) }}"
                            ...
                            <form action="{{ route('health-center.destroy-doctor', $doctor->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
