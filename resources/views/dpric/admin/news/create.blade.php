...
@section('content')
    ...
    <section class="w-full">
        <div class="w-11/12 mx-auto">
            ...
            <form action="{{ route('admin.dpric-news.store') }}" method="POST" enctype="multipart/form-data" class="w-full">
                @csrf
            <h3 class="oswald text-center my-4 text-2xl text-blue-800">Add News</h3>
                ...
                <div class="flex flex-col w-full md:w-10/12 my-4">
                    ...
                    <select name="category" value="{{ old('category') }}" class="border border-blue-500 p-2 rounded-sm bg-blue-100 focus:outline-none focus:border-2 text-gray-700">
                        <option value="" class="text-default-text">-- select --</option>
                        <option value="Technology" class="text-default-text">Technology</option>
                        <option value="Sports" class="text-default-text">Sports</option>
                    </select>
                    @error('category')
                    <p class="text-lg text-red-600 my-2">{{ $message }}</p>
                @enderror
