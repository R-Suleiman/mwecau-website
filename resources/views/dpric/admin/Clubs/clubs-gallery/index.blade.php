    ...
    <section class="w-full py-8">
        <div class="w-11/12 mx-auto">
            <div class="w-full flex flex-col md:flex-row justify-between items-center">
                <h2 class="oswald my-4 text-3xl text-blue-800">{{ $club_name }} Photo Gallery</h2>
                <a href="{{ route('admin.club-gallery.create', ['club_name' => $club_name]) }}"><button
                ...
                @foreach ($gallery as $image)
                    <div class="w-1/2 my-4 lg:w-1/4">
                        <div class="relative w-11/12 mx-auto overflow-hidden group h-52">
                            <img src="{{ asset('/storage/images/dpric/clubs/clubs-gallery/' . $image->image) }}" alt="gallery photo"
                        ...
                        <div class="mt-2 w-fit mx-auto mb-2 flex">
                            <form action="{{ route('admin.club-gallery.destroy', ['club_name' => $club_name, 'club_gallery' => $image]) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button
