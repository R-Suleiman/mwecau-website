...
@section('content')
    <section class="w-full">
        <div class="w-11/12 mx-auto">
            ...
            <form action="{{ route('admin.dpric-clubs.store') }}" method="POST" enctype="multipart/form-data" class="w-full">
                @csrf
                <h3 class="oswald text-center my-4 text-2xl text-blue-800">Register Club</h3>
                    ...
                    <select name="department" value="{{ old('department') }}"
                        ...
                        @foreach ($faculties as $faculty)
                            @foreach ($faculty['departments'] as $department)
                                <option value="{{ $department['dept_name'] }}" class="text-default-text">
                ...
                <div class="flex flex-col w-full md:w-10/12 my-4">
                    ...
                    <input type="date" name="found_date" value="{{ old('found_date') }}"
                        class="border border-blue-500 p-2 rounded-sm bg-blue-100 focus:outline-none focus:border-2 text-gray-700">
                    @error('found_date')
                        <p class="text-lg text-red-600 my-2">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex flex-col w-full md:w-10/12 my-4">
                    ...
                    <input type="text" name="total_members" value="{{ old('total_members') }}"
                        class="border border-blue-500 p-2 rounded-sm bg-blue-100 focus:outline-none focus:border-2 text-gray-700">
                    @error('total_members')
                        <p class="text-lg text-red-600 my-2">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex flex-col w-full md:w-10/12 my-4">
                    ...
                    <input type="text" name="total_projects" value="{{ old('total_projects') }}"
                        class="border border-blue-500 p-2 rounded-sm bg-blue-100 focus:outline-none focus:border-2 text-gray-700">
